...
- ネットワークドライブに表示する S3 バケットを名前や表示数で調整可能です。
- リードオンリーでのマウントを行えます。
...
4. 必要の無い場合は [WinFsp](https://winfsp.dev/rel/) をアンインストールする。

## 制限事項
- いくつかの制約については [設定ファイル](./doc/conf-example.txt) を変更することで緩和可能です。
- バケットの作成・削除などの操作は行えません。
- 不具合を検知しやすくするため、abort() を使用しているので強制終了する可能性があります。
